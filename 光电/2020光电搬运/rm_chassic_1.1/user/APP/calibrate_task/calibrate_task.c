...
static bool_t cali_gimbal_hook(uint32_t *cali, bool_t cmd)
{
    ...
    if (cmd == CALI_FUNC_CMD_INIT)  //校准函数，传递flash中的校准参数
    {
        set_cali_gimbal_hook(local_cali_t->yaw_offset, 
	  ...
	  else if (cmd == CALI_FUNC_CMD_ON)  //校准函数，使能校准
    {
      if (cmd_cali_gimbal_hook(&local_cali_t->yaw_offset, 
																 &local_cali_t->pitch_offset,
                                 &local_cali_t->yaw_max_angle, 
																 &local_cali_t->yaw_min_angle,
                                 &local_cali_t->pitch_max_angle, 
